...
# with regularization
lmbd = .5 # regularization coeff
X = np.concatenate([np.ones((len(x), 1)), x], axis=1).reshape(-1,2)
print(X.shape, np.linalg.inv(X.T @ X).shape)
A=X.T @ X
print(A , A+ (lmbd * np.eye(X.shape[1])))
print(lmbd * np.eye(X.shape[1]))
##w_lin = np.linalg.inv(X.T @ X + lmbd) @ X.T @ y
##num = 500
##X_lin = np.concatenate([np.ones((num, 1)), np.linspace(-.5, .5, num=500).reshape(-1,1)], axis=1).reshape(-1,2)
##y_hat_lin = X_lin @ w_lin
##
